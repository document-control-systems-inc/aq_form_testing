...
namespace NAPS2.ImportExport.Images
{
    public class SaveImagesOperation : OperationBase
    {
            ...
            thread = threadFactory.StartThread(() =>
            {
                try
                {
                    ...
                    if (Equals(format, ImageFormat.Tiff))
                    {
                        ...
                        {
                            subFileName = fileNamePlaceholders.SubstitutePlaceholders(subFileName, DateTime.Now, true, 1, 1);
                            /*
                        ...
                        //MessageBox.Show("Guardando TIFF: " + subFileName);
                        AquariusUtils.uploadFiles(new List<string>() { subFileName });
                        return;
                    }
                    ...
                    foreach (ScannedImage img in images)
                    {
                        ...
                        {
                            // se sustituye el nombre y se aumenta un numero diferencial
                            subFileName = fileNamePlaceholders.SubstitutePlaceholders(subFileName, DateTime.Now, true, 1, 1);
                            /*
        ...
        private void DoSaveImage(Bitmap image, string path, ImageFormat format)
        {
            ...
            //MessageBox.Show("Guardando imagen: " + path);
            AquariusUtils.uploadFiles(new List<string>() { path });
        }
