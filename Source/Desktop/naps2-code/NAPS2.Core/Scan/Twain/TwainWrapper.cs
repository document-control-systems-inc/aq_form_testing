        ...
        public List<ScannedImage> Scan(IWin32Window dialogParent, bool activate, ScanDevice scanDevice, ScanProfile scanProfile, ScanParams scanParams)
        {
            ...
            session.DataTransferred += (sender, eventArgs) =>
            {
                try
                {
                    ...
                    pageNumber++;
                    var output = GetBitmapFromMemXFer(eventArgs.MemoryData, eventArgs.ImageInfo);
                    //var output2 = Image.FromStream(eventArgs.GetNativeImageStream());
                    ...
                    */
                        using (var result = scannedImageHelper.PostProcessStep1(output, scanProfile))
                        {
                            ...
                            MessageBox.Show("Pixel Format" + output.PixelFormat);
