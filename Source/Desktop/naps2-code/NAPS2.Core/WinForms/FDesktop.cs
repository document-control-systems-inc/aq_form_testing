...
namespace NAPS2.WinForms
{
    public partial class FDesktop : FormBase
    {
        ...
        private void InitTransferImages()
        {
            if (!ResultadoLogin.bathLoaded)
            {
                ResultadoLogin.bathLoaded = true;
                List<String> lstrutaFiles = new List<String>();
                if (EntityNodo.LstrutaFiles != null)
                {
                    lstrutaFiles = EntityNodo.LstrutaFiles;
                }
                String path = AquariusUtils.getFilePathRecoveryCopia();
                if (Directory.Exists(path))
                {
                    foreach (string fileName in Directory.GetFiles(path))
                    {
                        if (Path.GetExtension(fileName).ToString().ToLower().Equals(".jpg")
                            || Path.GetExtension(fileName).ToString().ToLower().Equals(".jpeg")
                            || Path.GetExtension(fileName).ToString().ToLower().Equals(".bpm")
                            || Path.GetExtension(fileName).ToString().ToLower().Equals(".emf")
                            || Path.GetExtension(fileName).ToString().ToLower().Equals(".gif")
                            || Path.GetExtension(fileName).ToString().ToLower().Equals(".ico")
                            || Path.GetExtension(fileName).ToString().ToLower().Equals(".png")
                            || Path.GetExtension(fileName).ToString().ToLower().Equals(".tiff")
                            || Path.GetExtension(fileName).ToString().ToLower().Equals(".tif")
                            || Path.GetExtension(fileName).ToString().ToLower().Equals(".wmf")
                            || Path.GetExtension(fileName).ToString().ToLower().Equals(".pdf"))
                        {
                            if (!lstrutaFiles.Contains(fileName))
                            {
                                lstrutaFiles.Add(fileName);
                            }
                        }

                    }
                    EntityNodo.LstrutaFiles = lstrutaFiles;
                }
                if (lstrutaFiles.Count > 0)
                {
                    if (profileManager.DefaultProfile == null)
                    {
                        //Abrir Treeview
                        EntityNodo.isLogin = true;
                        var form = FormFactory.Create<FTreeView>();
                        form.ShowDialog();
                    } else
                    {
                        EntityNodo.cancelUpdate = false;
                        EntityNodo.Id = profileManager.DefaultProfile.Repository.IdParent;
                        EntityNodo.Nombre = profileManager.DefaultProfile.Repository.NameParent;
                        EntityNodo.DocumentClass = profileManager.DefaultProfile.Repository.DocumentClass;
                        EntityNodo.Domain = profileManager.DefaultProfile.Repository.Domain;

                    }
                    if (!EntityNodo.cancelUpdate)
                    {
                        AquariusUtils.uploadFiles(EntityNodo.LstrutaFiles);
                    }
                }
            }
        }
        ...
        protected override void OnLoad(object sender, EventArgs eventArgs)
        {
            ...
            //MessageBox.Show(ResultadoLogin.exito);
        }
        ...
        private void InitLanguageDropdown()
        {
            ...
            foreach (DictionaryEntry entry in resourceSet.Cast<DictionaryEntry>().OrderBy(x => x.Value))
            {
                ...
                else if (langCode == "es" || File.Exists(localizedResourcesPath))
                {
                    var button = new ToolStripMenuItem(langName, null, (sender, args) => SetCulture(langCode));
                    toolStripDropDownButton1.DropDownItems.Add(button);
                }
        ...
        public void ReceiveScannedImage(ScannedImage scannedImage)
        {
            ...
            SafeInvoke(() =>
            {
                ...
                // Se comenta lo de copiar a la ruta original
                /*
                //copiar imagen a otra ruta
                String path_copy = AquariusUtils.getFilePathRecoveryCopia();
                String imgCopy = path_copy + "\\" + System.Guid.NewGuid().ToString() + Path.GetExtension(scannedImage.RecoveryFilePath);
                File.Copy(scannedImage.RecoveryFilePath, imgCopy);

                lstrutaFiles.Add(imgCopy);
                EntityNodo.LstrutaFiles = lstrutaFiles;
                //MessageBox.Show(scannedImage.RecoveryFilePath.ToString());
                */
        ...
        private void SavePDF(List<ScannedImage> images)
        {
            ...
            //MessageBox.Show(ResultadoLogin.rutaSave);
        }
        ...
        private void tsAbout_Click(object sender, EventArgs e)
        {
            ...
            if (locale.Equals("es"))
            {
                messageBoxText = "¿Está seguro de salir de la aplicación?";
                messageBoxTitle = "Confirmación";
            } else
            {
                messageBoxText = "Do you really want to exit?";
                messageBoxTitle = "Confirmation";
            }
            DialogResult result = MessageBox.Show(messageBoxText, messageBoxTitle, MessageBoxButtons.YesNo);
            if (result == DialogResult.Yes)
            {
                //String locale = "es_mx";
                if (CloseSession(ResultadoLogin.token, locale))
                {
                    Environment.Exit(0);
                }
        ...
        //Cerrar sesión:
        //Se cierra sesión
        private Boolean CloseSession(String token, String locale)
        {
            ...
            try
            {
                ...
                using (Stream responseStream = response.GetResponseStream())
                {
                    ...
                    String resultadoError = MensajeError(locale, jObject.GetValue("status").ToString());
